...
int CKoopaGreenNormal::GetAniId()
{
		...
		state == KOOPA_STATE_SHELL_MARIO_HOLD_DOWNSIDE)
	{
		if (GetTickCount64() - shell_start > KOOPA_SHELL_SHAKE_TIME)
			return ID_ANI_KOOPA_GREEN_NORMAL_SHELL_SHAKE_DOWNSIDE;
		else
	...
	{
		if (GetTickCount64() - shell_start > KOOPA_SHELL_SHAKE_TIME)
			return ID_ANI_KOOPA_GREEN_NORMAL_SHELL_SHAKE_UPSIDE;
		else
			return ID_ANI_KOOPA_GREEN_NORMAL_SHELL_STILL_UPSIDE;
	}
	if (state == KOOPA_STATE_DIE) return ID_ANI_KOOPA_GREEN_NORMAL_SHELL_STILL_UPSIDE;
